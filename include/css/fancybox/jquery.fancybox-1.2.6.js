...
;(function(jQuery) {
	...
	var ieQuirks = null, IE6 = jQuery.browser.msie && jQuery.browser.version.substr(0,1) == 6 && !window.XMLHttpRequest, oldIE = IE6 || (jQuery.browser.msie && jQuery.browser.version.substr(0,1) == 7);

	jQuery.fn.fancybox = function(o) {
		var settings		= jQuery.extend({}, jQuery.fn.fancybox.defaults, o);
		var matchedGroup	= this;

		function _initialize() {
			elem = this;
			opts = jQuery.extend({}, settings);
		...
		function _start() {
			...
			jQuery(window).bind("resize.fb scroll.fb", jQuery.fn.fancybox.scrollBox);
		...
		function _set_content(value, width, height) {
			...
			if (pad > 0) {
				...
				if (oldIE || ieQuirks) {
					jQuery("#fancy_content")[0].style.setExpression('height',	'(this.parentNode.clientHeight - '	+ pad * 16 + ')'); // Xolo, S.A.
					//jQuery("#fancy_content")[0].style.setExpression('height',	'(this.parentNode.clientHeight - '	+ pad * 2 + ')'); // Original
		...
		function _finish() {
			...
			if (opts.overlayShow && opts.hideOnOverlayClick) {
				jQuery("#fancy_overlay").bind("click", jQuery.fn.fancybox.close);
			}

			if (opts.showCloseButton) {
				jQuery("#fancy_close").bind("click", jQuery.fn.fancybox.close).show();
			}
	...
	jQuery.fn.fancybox.build = function() {
		...
		html += '<div id="fancy_inner"><div id="fancy_header"></div>';
